...
# Version 2.4 made by CrazyCloudCraft 10.03.2022 https://crazycloudcraft.de
...
# You can use: PAPER, PURPUR, MOHIST, VELOCITY, BUNGEECORD, SPIGOT 
...
if [ $ASOFTWARE = "BUNGEECORD" ]; then
 cd $LPATH/jar
 wget -q https://ci.md-5.net/job/BungeeCord/lastSuccessfulBuild/artifact/bootstrap/target/BungeeCord.jar -O BungeeCord.jar
 unzip -qq -t BungeeCord.jar
 if [ "$?" -ne 0 ]; then
  echo "Downloaded BungeeCord.jar No update." | /usr/bin/logger -t $MCNAME
 else
  diff -q BungeeCord.jar ../BungeeCord.jar >/dev/null 2>&1
  if [ "$?" -eq 1 ]; then
   /usr/bin/find $LPATH/jar/* -type f -mtime +10 -delete 2>&1 | /usr/bin/logger -t $MCNAME
   cp BungeeCord.jar BungeeCord.jar.$(date +%Y.%m.%d.%H.%M.%S)
   mv BungeeCord.jar $LPATH/BungeeCord.jar
...
if [ $ASOFTWARE = "BUNGEECORD" ]; then
 screen -d -m -L -S $MCNAME  /bin/bash -c "$JAVABIN -Xms$RAM -Xmx$RAM -XX:+UseG1GC -XX:G1HeapRegionSize=4M -XX:+UnlockExperimentalVMOptions -XX:+ParallelRefProcEnabled -XX:+AlwaysPreTouch -XX:MaxInlineLevel=15 -jar BungeeCord.jar"
 exit 0
fi
